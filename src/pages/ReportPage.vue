      ...
      <div class="column">
        ...
        <div class="max-width">
          ...
          <section v-else class="max-width">
            ...
            <td v-if="props.row.data && props.row.type == 'address'" :data-label="$t('lang.common.value-row')" class="flex-mobile text-wrap">
                  <a :href="$libre.addressToLink(props.row.data)" class="is-text-overflow">{{ props.row.data }}</a>
            </td> 
            <b-table-column v-if="props.row.data && props.row.type != 'address'" :label="$t('lang.reports.value-row')" class="flex-mobile flex-wrap text-wrap">
              {{ props.row.data }}
            </b-table-column>
          </template>
        </b-table>
...
export default {
  ...
  methods: {
    ...
    async getReport(i) {
      let raw = await this.$libre.report.reports(i),
        result;
      try {
      ...
      result.date = +raw[1] == 0 ? "-" : this.$d(raw[1] * 1000, "long");
      console.log(result);
      return result;
    },
