...
export default {
  data() {
    return {
      ...
      msg: {
        type: "is-info",
        text: t('lang.common.loading-info-dots')
      },
      isDisabled: true
    };
  },
  ...
  methods: {
    async loadLiberty(got = false) {
      ...
      if (got) {
        this.msg.text = t('lang.faucet.tokens-sent');
      } else if (!isGot && balance >= 2000 && this.$store.state.address) {
        this.isDisabled = false;
        this.msg.text = t('lang.faucet.yes-you-can');
      } else if (!this.$store.state.address) {
        this.msg = {
          type: "is-danger",
          text: t('lang.common.no-metamask')
        };
      } else {
        this.msg = {
          type: "is-danger",
          text: isGot
            ? t('lang.faucet.already-sent')
            : t('lang.faucet.not-enough')
        };
      }
    },
    ...
    async getTokens() {
      ...
      try {
        ...
        if (await this.$eth.isSuccess(txHash)) {
          this.$libre.notify(t('lang.faucet.tokens-sent'));
          this.loadLiberty(true);
        } else {
          this.$libre.notify(t('lang.faucet.error-sending'),'is-info');
        }
